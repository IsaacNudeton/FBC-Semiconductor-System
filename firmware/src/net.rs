...
// =============================================================================
// Simple TCP Server (without smoltcp for initial testing)
// =============================================================================

/// Minimal IP/TCP implementation for testing
/// Real deployment should use smoltcp
...
impl TcpServer {
    ...
    pub fn init(&mut self) {
        self.eth.init(&self.config);
    }

    /// Poll for incoming packets
    /// Returns a slice of the received payload if a valid TCP packet arrived
    pub fn poll(&mut self) -> Option<&[u8]> {
        let len = self.eth.recv(&mut self.rx_buf);
        if len == 0 {
            return None;
        }
        ...
        let ihl = (self.rx_buf[ip_start] & 0x0F) as usize * 4;
        let protocol = self.rx_buf[ip_start + 9];
        ...
        let dst_port = u16::from_be_bytes([self.rx_buf[tcp_start + 2], self.rx_buf[tcp_start + 3]]);
        if dst_port != self.config.port {
            return None;
        }

        let data_offset = ((self.rx_buf[tcp_start + 12] >> 4) as usize) * 4;
        let payload_start = tcp_start + data_offset;

        if payload_start < len {
            Some(&self.rx_buf[payload_start..len])
        } else {
            None
        }
    }
    ...
    fn handle_arp(&mut self, len: usize) {
        ...
        let arp_start = 14;
        let opcode = u16::from_be_bytes([self.rx_buf[arp_start + 6], self.rx_buf[arp_start + 7]]);
        if opcode != 1 {  // Not a request
            return;
        }

        // Check target IP
        let target_ip = &self.rx_buf[arp_start + 24..arp_start + 28];
        if target_ip != self.config.ip {
            return;
        }

        // Build ARP reply
        let sender_mac = &self.rx_buf[arp_start + 8..arp_start + 14];
        let sender_ip = &self.rx_buf[arp_start + 14..arp_start + 18];

        // Ethernet header
        self.tx_buf[0..6].copy_from_slice(sender_mac);  // Dest MAC
        self.tx_buf[6..12].copy_from_slice(&self.config.mac);  // Src MAC
        self.tx_buf[12..14].copy_from_slice(&[0x08, 0x06]);  // ARP

        // ARP reply
        self.tx_buf[14..16].copy_from_slice(&[0x00, 0x01]);  // Hardware type: Ethernet
        self.tx_buf[16..18].copy_from_slice(&[0x08, 0x00]);  // Protocol: IPv4
        self.tx_buf[18] = 6;  // Hardware size
        self.tx_buf[19] = 4;  // Protocol size
        self.tx_buf[20..22].copy_from_slice(&[0x00, 0x02]);  // Opcode: Reply
        self.tx_buf[22..28].copy_from_slice(&self.config.mac);  // Sender MAC
        self.tx_buf[28..32].copy_from_slice(&self.config.ip);  // Sender IP
        self.tx_buf[32..38].copy_from_slice(sender_mac);  // Target MAC
        self.tx_buf[38..42].copy_from_slice(sender_ip);  // Target IP

        self.eth.send(&self.tx_buf[..42]);
    }

    /// Send a response
    pub fn send(&mut self, data: &[u8]) -> bool {
        // For now, just send raw ethernet frame
        // Full implementation needs IP/TCP header construction
        self.eth.send(data)
    }
}

// =============================================================================
// UDP Support (No TCP overhead)
// =============================================================================

/// Simple UDP packet builder
...
impl UdpPacket {
    ...
    pub fn new(
        ...
        if payload.len() > 1472 {
            // Max UDP payload in standard Ethernet frame
            return None;
        }
        ...
        pkt.buf[0..6].copy_from_slice(&dst_mac);
        pkt.buf[6..12].copy_from_slice(&src_mac);
        pkt.buf[12..14].copy_from_slice(&[0x08, 0x00]); // IPv4
        offset += 14;

        // IP header (20 bytes, no options)
        let ip_len = 20 + 8 + payload.len();
        pkt.buf[offset] = 0x45;  // IPv4, IHL=5
        pkt.buf[offset + 1] = 0;  // DSCP/ECN
        pkt.buf[offset + 2] = (ip_len >> 8) as u8;
        pkt.buf[offset + 3] = (ip_len & 0xFF) as u8;
        pkt.buf[offset + 4..offset + 6].copy_from_slice(&[0, 0]); // ID
        pkt.buf[offset + 6..offset + 8].copy_from_slice(&[0, 0]); // Flags/Fragment
        pkt.buf[offset + 8] = 64;  // TTL
        pkt.buf[offset + 9] = 17;  // Protocol: UDP
        pkt.buf[offset + 10..offset + 12].copy_from_slice(&[0, 0]); // Checksum (calculate later)
        pkt.buf[offset + 12..offset + 16].copy_from_slice(&src_ip);
        pkt.buf[offset + 16..offset + 20].copy_from_slice(&dst_ip);

        // Calculate IP checksum
        let ip_checksum = Self::checksum(&pkt.buf[offset..offset + 20]);
        pkt.buf[offset + 10] = (ip_checksum >> 8) as u8;
        pkt.buf[offset + 11] = (ip_checksum & 0xFF) as u8;
        offset += 20;

        // UDP header (8 bytes)
        let udp_len = 8 + payload.len();
        pkt.buf[offset] = (src_port >> 8) as u8;
        pkt.buf[offset + 1] = (src_port & 0xFF) as u8;
        pkt.buf[offset + 2] = (dst_port >> 8) as u8;
        pkt.buf[offset + 3] = (dst_port & 0xFF) as u8;
        pkt.buf[offset + 4] = (udp_len >> 8) as u8;
        pkt.buf[offset + 5] = (udp_len & 0xFF) as u8;
        pkt.buf[offset + 6..offset + 8].copy_from_slice(&[0, 0]); // Checksum (optional for UDP)
        offset += 8;

        // Payload
        pkt.buf[offset..offset + payload.len()].copy_from_slice(payload);
        offset += payload.len();
...
/// Raw Ethernet frame builder (no IP/UDP, for custom protocols)
pub struct RawFrame {
    buf: [u8; 1536],
    len: usize,
}
...
impl RawFrame {
    /// Create new raw Ethernet frame
    pub fn new(dst_mac: [u8; 6], src_mac: [u8; 6], ethertype: u16, payload: &[u8]) -> Option<Self> {
        if payload.len() > 1500 {
            return None;
        }
        ...
        // Destination MAC
        frame.buf[0..6].copy_from_slice(&dst_mac);
        // Source MAC
        frame.buf[6..12].copy_from_slice(&src_mac);
        // EtherType
        frame.buf[12] = (ethertype >> 8) as u8;
        frame.buf[13] = (ethertype & 0xFF) as u8;
        // Payload
        frame.buf[14..14 + payload.len()].copy_from_slice(payload);

        frame.len = 14 + payload.len();
        Some(frame)
    }
